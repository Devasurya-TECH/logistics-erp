...
import { CheckCircleIcon, XCircleIcon, ClockIcon, MapPinIcon, PhoneIcon } from "@heroicons/react/24/outline";
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md uppercase tracking-tighter border border-emerald-100">
                        ✨ Optimized
                    </span>
                </div>
                        ...
                        <div key={drop.id} className="relative pl-4 md:pl-6 animate-fade-in-up" style={{ animationDelay: `${index * 0.05}s` }}>
                            ...
                            <div className={`p-4 md:p-5 rounded-2xl border transition-all duration-300 ${isNext ? 'bg-white border-blue-200 shadow-lg shadow-blue-500/10' :
                                ...
                                <div className="space-y-2 mb-4">
                                    <p className="text-xs md:text-sm text-slate-500 font-medium flex items-start gap-1.5 line-clamp-2">
                                        <MapPinIcon className="w-3.5 h-3.5 flex-shrink-0 mt-0.5 text-slate-400" />
                                        {drop.address}
                                    </p>
                                    {drop.customerPhone && (
                                        <div className="flex items-center justify-between bg-slate-100/50 p-2.5 rounded-xl border border-dotted border-slate-200">
                                            <div className="flex items-center gap-2">
                                                <div className="bg-slate-200 p-1.5 rounded-lg text-slate-600">
                                                    <PhoneIcon className="w-3.5 h-3.5" />
                                                </div>
                                                <span className="text-[11px] font-bold text-slate-600 tracking-tight">{drop.customerPhone}</span>
                                            </div>
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    window.open(`tel:${drop.customerPhone}`);
                                                }}
                                                className="text-[10px] font-black text-blue-600 uppercase tracking-tighter bg-white px-3 py-1.5 rounded-lg shadow-sm border border-blue-100 active:scale-95 transition-all"
                                            >
                                                Call Now
                                            </button>
                                        </div>
